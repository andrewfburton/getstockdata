...
function getHistoricalStockData(ticker, startDate, endDate) {
  ...
  getStock({ stock: ticker, startDate: startDate, endDate: endDate }, 'historicaldata', function(err, data) {
	...
	var i = 0;
	//Fixed > to >=, otherwise last day is not included
   for(var i = data.quote.length-1; i >= 0; i--) {
      //debugger;
      var tempObj = {};
      tempObj.date = new Date(data.quote[i].Date);
	  tempObj.open = Number(data.quote[i].Open).toFixed(2);
	  tempObj.high = Number(data.quote[i].High).toFixed(2);
	  tempObj.low = Number(data.quote[i].Low).toFixed(2);
      tempObj.close = Number(data.quote[i].Close).toFixed(2);
	  //tempObj.volume = Number(data.quote[i].Volume);
